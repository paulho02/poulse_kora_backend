...
class TestOperationRetry:
    async def test_reschedule_moves_due_op_to_front_of_ops(self, redis: Redis):
        # Simulate a stalled op (post 1) plus a newer op (post 2) already queued.
        ...
        assert moved == 1
        # Retried op re-inserted at the head ⇒ still popped before the newer one.
        assert await redis.lrange(keys.OPS, 0, -1) == [
            '{"post_id": 1, "channel_id": 1}',
            '{"post_id": 2, "channel_id": 1}',
        ]
    ...
    async def test_reschedule_ignores_not_yet_due(self, redis: Redis):
        ...
        assert await redis.llen(keys.OPS) == 0
    ...
    async def test_schedule_retry_disambiguates_same_post_and_channel(
        ...
        moved = await service.reschedule_due_retries(redis)
        assert moved == 2
        assert await redis.llen(keys.OPS) == 2
